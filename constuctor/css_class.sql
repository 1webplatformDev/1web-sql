...
--select * from constuctor.css_class_check_unieue;
--select * from constuctor.css_class_insert;
...
drop table if exists constuctor.css_class cascade;
-- alter sequence constuctor.css_class_id_seq restart with 1;
...
comment on column constuctor.css_class.class_name is 'Имя css class';
comment on column constuctor.css_class.description is 'Описание css class';
comment on column constuctor.css_class.active is 'Активность css class';

-- function

drop function if exists constuctor.css_class_get_filter;
create or replace function constuctor.css_class_get_filter(
	_id int4 = null,
	...
	returns SETOF constuctor.css_class
	language plpgsql
	as $function$
	begin
		return query 
			select * from constuctor.css_class cc
...
drop function if exists constuctor.css_class_check_unieue;
create or replace function constuctor.css_class_check_unieue(
	in _id int4 = null,
	...
	declare
		...
		error_array int[];
	begin
		select count(*) into count_class_name from constuctor.css_class_get_filter(_class_name => _class_name, _no_id => _id);
...
drop function if exists constuctor.css_class_insert;
create or replace function constuctor.css_class_insert(
	...
	as $function$
	begin
		select * into result_ from constuctor.type_component_check_id(_id => _id_type_component);
		if (result_::json->'status_result')::text::int = 404 then
			return;
		end if;

		select * into result_ from constuctor.css_class_check_unieue(_class_name => _class_name);
		if (result_::json->'status_result')::text::int = 200 then
			insert into constuctor.css_class (id_type_component, class_name, description, active)
...
drop function if exists constuctor.css_class_updated;
create or replace function constuctor.css_class_updated(
	in _id int4,
	in _id_type_component int4,
	in _class_name varchar,
	...
	as $function$
	begin
		select * into result_ from constuctor.css_class_check_id(_id => _id);
		if (result_::json->'status_result')::text::int = 404 then
			return;
		end if;
		...
		select * into result_ from constuctor.css_class_check_unieue(_class_name => _class_name, _id => _id);
		if (result_::json->'status_result')::text::int = 200 then
			update constuctor.css_class
...
drop function if exists constuctor.css_class_check_id;
create or replace function constuctor.css_class_check_id(
	in _id int4,
	out result_ json
)
	language plpgsql
	...
	begin
		...
		select count(*) into check_rows from constuctor.css_class_get_filter(_id => _id);
